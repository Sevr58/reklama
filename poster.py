...
async def _send_to_telegram(text: str, image_path: str = None):
    ...
    if image_path:
        with open(image_path, "rb") as photo:
            if len(text) <= 1024:
                # Короткий текст — отправляем как подпись к фото
...
def post_social(text: str, image_path: str = None, platforms: list = None):
    """Публикует пост в Instagram, Facebook через upload-post.com."""
    if platforms is None:
        platforms = ["instagram", "facebook"]

    # Instagram принимает максимум 2200 символов
    caption = text[:2200]
    ...
    try:
        if image_path:
            ...
            # Текстовый пост (только Facebook/LinkedIn)
            data = {
                "user": UPLOAD_POST_USERNAME,
                "title": caption,
            }
        ...
        log.warning(f"[Social] Ошибка публикации: {e}")
